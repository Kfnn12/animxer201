...
const targetStr = '<div className="mb-6 flex items-center justify-between border-b border-white/10 pb-4">\\n                    <h2 className="text-lg font-semibold text-white">Episodes</h2>\\n                    <span className="text-sm text-[#A0A0A0]">{animeInfo.episodes.length} episodes</span>\\n                  </div>';

const replaceStr = `<div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/10 pb-4 gap-4">
...
let newStr = content.replace(targetStr, replaceStr).replace(targetStrCRLF, replaceStr);
