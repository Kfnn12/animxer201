...
const regex = /<div className="mb-6 flex items-center justify-between border-b border-white\/10 pb-4">\s*<h2 className="text-lg font-semibold text-white">Episodes<\/h2>\s*<span className="text-sm text-\[#A0A0A0\]">\{animeInfo\.episodes\.length\} episodes<\/span>\s*<\/div>/;

const replace2 = `<div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/10 pb-4 gap-4">
...
let newCode = code.replace(regex, replace2);
