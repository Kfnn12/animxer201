...
import { Search, Play, ArrowLeft, Loader2, Info, MonitorPlay, Mic, Captions, ServerIcon, AlertCircle, Bug, CheckCircle, ChevronLeft, ChevronRight, TrendingUp, Download, Copy, Check, Calendar, ServerCrash } from "lucide-react";
import type { AnimeResult, AnimeInfo, Episode, Server, TrendingAnime, AiringScheduleItem, NextAiringEpisode } from "./types";
import Hls from "hls.js";
import { motion, AnimatePresence } from "motion/react";

const HlsPlayer = ({ src, tracks = [] }: { src: string, tracks?: any[] }) => {
  ...
  useEffect(() => {
    ...
    const enableSubtitles = () => {
      ...
      for (let i = 0; i < textTracks.length; i++) {
        ...
        if ((track.label && track.label.toLowerCase().includes('english')) || track.language === 'en') {
          track.mode = 'showing';
        } else {
          track.mode = 'hidden';
        }
  ...
  return (
    <video ref={videoRef} className="w-full h-full border-0 relative z-10" controls autoPlay playsInline crossOrigin="anonymous">
      {tracks.map((track, i) => (
        <track 
          key={i}
          kind={track.kind || "captions"} 
          src={track.file?.startsWith('http') ? `/api/proxy?url=${encodeURIComponent(track.file)}` : track.file} 
          srcLang={track.label?.toLowerCase().substring(0, 2) || "en"} 
          label={track.label} 
          default={track.default || (track.label && track.label.toLowerCase().includes('english'))} 
        />
      ))}
    </video>
  );
...
export default function App() {
  ...
  const observerRef = React.useCallback((node: HTMLDivElement | null) => {
    ...
    currentObserver.current = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && hasMoreRecent && view === "search" && !query) {
  ...
  const [selectedAnime, setSelectedAnime] = useState<AnimeResult | TrendingAnime | null>(null);
  ...
  const getInfo = async (anime: AnimeResult | TrendingAnime) => {
    ...
    try {
      ...
      fetch(`/api/schedule?title=${encodeURIComponent(data.title.split(' (')[0])}`)
        .then(r => r.json())
        .then(d => {
  ...
  const [nextEpisode, setNextEpisode] = useState<NextAiringEpisode | null>(null);
  ...
  const watchEpisode = async (ep: Episode) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to fetch servers. The upstream provider might be down.");
      }
      
      if (!data.servers || data.servers.length === 0) {
        throw new Error("No servers are available for this specific episode. Please check back later.");
      }
      setServers(data.servers || []);
      setLoading(false); // Disable main loading before selecting server
      
      // Auto select first sub server if available
      const firstSub = data.servers.find((s: Server) => s.type === "sub" && s.name.toLowerCase().includes("vidstreaming")) || data.servers.find((s: Server) => s.type === "sub") || data.servers[0];
      if (firstSub) {
        selectServer(firstSub);
      }
    } catch (err: any) {
      console.error(err);
      setError(err.message || "A network error occurred while loading servers.");
      setLoading(false);
    }
  };
  ...
  const selectServer = async (server: Server) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Video source provider returned an error.");
      }
      
      if (!data.link) throw new Error("Video source link was empty. Try a different server.");
      ...
      console.error(err);
      setError(err.message || "Could not load the video from this server. Please try a different server or refresh the page.");
  ...
  const handleReportIssue = () => {
    ...
    const reportData = {
        ...
        episode: selectedEpisode ? `EP ${selectedEpisode.number} - ${selectedEpisode.title}` : null,
      ...
      <header className="sticky top-0 z-50 bg-[#050505]/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
          ...
          >
            ...
            <span className="font-bold text-xl text-white tracking-tight">ANIMXER</span>
          </button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center p-12 text-[#FF3E3E] bg-[#FF3E3E]/5 rounded-2xl border border-[#FF3E3E]/20 max-w-2xl mx-auto text-center">
                 ...
                 <div className="flex flex-wrap items-center justify-center gap-3">
                   ...
                   >
                     {reportState === "idle" && <><Bug className="w-4 h-4" /> Report Issue</>}
                     {reportState === "reporting" && <Loader2 className="w-4 h-4 animate-spin" />}
                     {reportState === "reported" && <><CheckCircle className="w-4 h-4" /> Reported!</>}
                   </button>
                 </div>
              </div>
            ) : results.length > 0 ? (
              ...
              <div className="animate-in fade-in duration-1000">
                {trending.length > 0 && (
                  <div className="mb-12 relative rounded-2xl overflow-hidden aspect-[21/9] sm:aspect-[21/7] bg-[#121212] group">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveSlideIndex((prev) => (prev - 1 + trending.length) % trending.length);
                      }}
                  ...
                  <div className="mb-12">
                     ...
                     <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar snap-x">
                        {schedules.map((schedule, idx) => (
                           <div key={idx} className="snap-start shrink-0 w-64 md:w-72 bg-[#121212] rounded-xl overflow-hidden border border-white/5 shadow-lg group">
                              ...
                              <div className="p-4 pt-2">
                                 <h3 className="text-white font-semibold line-clamp-1 mb-2 group-hover:text-[#FF3E3E] transition-colors">{schedule.title}</h3>
                                 <div className="flex justify-between items-center text-xs">
                                     <span className="text-[#A0A0A0] font-medium">
                                        {new Intl.DateTimeFormat(navigator.language, { weekday: 'short', hour: 'numeric', minute: '2-digit' }).format(new Date(schedule.time * 1000))}
                                     </span>
                                     <span className="text-zinc-500">
                                        {new Date(schedule.time * 1000).toLocaleDateString()}
                                     </span>
                                 </div>
                              </div>
                           </div>
                ...
                {recentUpdates.length > 0 && hasMoreRecent && (
                  <div ref={observerRef} className="py-8 flex justify-center mt-4">
                    {recentLoading && <Loader2 className="w-8 h-8 text-[#FF3E3E] animate-spin" />}
                  </div>
                )}
              </div>
            ) : (
        ...
        {view === "info" && selectedAnime && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {loading ? (
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="w-full md:w-64 flex-shrink-0">
                  <div className="aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 relative">
                    <img src={animeInfo.poster} alt={animeInfo.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {nextEpisode && (
                    <div className="mb-8 p-4 rounded-xl bg-[#FF3E3E]/10 border border-[#FF3E3E]/20 flex items-center justify-between shadow-lg">
                       <div>
                         <h3 className="text-[#FF3E3E] font-bold text-sm uppercase tracking-wider mb-1">Airing Next</h3>
                         <p className="text-white">Episode {nextEpisode.episode}</p>
                       </div>
                       <div className="text-right">
                         <div className="text-sm font-medium text-white/90">
                           {new Intl.DateTimeFormat(navigator.language, { weekday: 'long', hour: 'numeric', minute: '2-digit' }).format(new Date(nextEpisode.airingAt * 1000))}
                         </div>
                  ...
                  <div className="mb-6 flex items-center justify-between border-b border-white/10 pb-4">
                    ...
                    <span className="text-sm text-[#A0A0A0]">{animeInfo.episodes.length} episodes found</span>
                  </div>
             ...
             {view === "info" && animeInfo?.recommended && animeInfo.recommended.length > 0 && (
               <div className="mt-12">
                 <h2 className="text-xl font-bold text-white mb-6 border-l-4 border-[#FF3E3E] pl-3">Recommended For You</h2>
        ...
        {view === "watch" && selectedAnime && selectedEpisode && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto">
             ...
             <div className="bg-[#050505] rounded-2xl overflow-hidden border border-white/10 shadow-2xl mb-8">
                 <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#121212] gap-2 flex-wrap sm:flex-nowrap">
                  <h2 className="text-lg font-medium text-white flex items-center gap-2 max-w-full sm:max-w-[50%]">
                    <span className="text-[#FF3E3E] font-bold shrink-0">EP {selectedEpisode.number}</span>
                    <span className="text-[#A0A0A0] text-sm mx-1 shrink-0">&bull;</span>
                    <span className="truncate">{selectedEpisode.title || selectedAnime.title}</span>
                  </h2>
                 ...
                 {loading || serverLoading && !error ? (
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center text-[#FF3E3E] gap-3 p-6 text-center bg-[#121212]/50">
                     ...
                     <div className="flex flex-wrap items-center justify-center gap-3 mt-4">
                       <button 
                         onClick={() => selectedServer ? selectServer(selectedServer) : selectedEpisode && watchEpisode(selectedEpisode)} 
                   ...
                   proxyNeeded ? (
                     <HlsPlayer src={`/api/proxy?url=${encodeURIComponent(sourceLink)}`} tracks={sourceTracks} />
                   ) : (
                     <>
                       <iframe 
                         src={sourceLink} 
                       ...
                       <div className="absolute inset-0 flex flex-col items-center justify-center text-[#A0A0A0] text-center p-8 z-0">
                          ...
                          <p className="mb-2">If the video fails to load due to <strong>browser streaming restrictions</strong>,</p>
                          <a href={externalWatchUrl} target="_blank" rel="noopener noreferrer" className="text-[#FF3E3E] hover:underline underline-offset-4">Click here to watch it safely in a new tab</a>
                       </div>
                     </>
                   )
                 ) : (
               ...
               <div className="p-4 md:p-6 bg-[#121212]">
                 <div className="mb-4">
                   <h3 className="text-sm text-[#A0A0A0] font-medium uppercase tracking-wider">Available Servers</h3>
                 </div>
                 {error && servers.length === 0 ? (
                   <div className="text-sm text-[#FF3E3E] p-4 bg-[#FF3E3E]/10 rounded-lg border border-[#FF3E3E]/20 flex flex-col items-center justify-center gap-3 text-center">
                     ...
                     <div className="flex flex-wrap items-center justify-center gap-3 mt-2">
                       ...
                       >
                         {reportState === "idle" && <><Bug className="w-3.5 h-3.5" /> Report Issue</>}
                         {reportState === "reporting" && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                         {reportState === "reported" && <><CheckCircle className="w-3.5 h-3.5" /> Reported!</>}
                       </button>
                     </div>
                   </div>
                 ) : servers.length > 0 ? (
                   <div className="flex flex-col gap-6">
                     ...
                     {servers.some(s => s.type === 'sub') && (
                       <div className="flex flex-col sm:flex-row gap-4 sm:items-start">
                         ...
                         <div className="flex flex-wrap gap-3">
                           {servers.filter(s => s.type === 'sub').map((server) => (
                             <button
                               key={server.id}
                               onClick={() => selectServer(server)}
                               title={serverErrors[server.id] ? "Server reported an error. Click to retry." : ""}
                               disabled={serverLoading && selectedServer?.id === server.id}
                             ...
                             >
                               {serverLoading && selectedServer?.id === server.id ? (
                                 ...
                                 <ServerIcon className={`w-4 h-4 transition-colors ${selectedServer?.id === server.id ? "text-white" : "text-[#FF3E3E]/60 group-hover:text-[#FF3E3E]"}`} />
                               )}
                               {server.name}
                             </button>
                           ))}
                     ...
                     {servers.some(s => s.type === 'dub') && (
                       <div className="flex flex-col sm:flex-row gap-4 sm:items-start">
                         ...
                         <div className="flex flex-wrap gap-3">
                           {servers.filter(s => s.type === 'dub').map((server) => (
                             <button
                               key={server.id}
                               onClick={() => selectServer(server)}
                               title={serverErrors[server.id] ? "Server reported an error. Click to retry." : ""}
                     ...
                     {servers.some(s => s.type === 'raw') && (
                       <div className="flex flex-col sm:flex-row gap-4 sm:items-start">
                         ...
                         <div className="flex flex-wrap gap-3">
                           {servers.filter(s => s.type === 'raw').map((server) => (
                             <button
                               key={server.id}
                               onClick={() => selectServer(server)}
                               title={serverErrors[server.id] ? "Server reported an error. Click to retry." : ""}
                   ...
                   <div className="flex flex-col items-center justify-center p-8 text-[#A0A0A0] bg-white/5 rounded-xl border border-white/10 text-center gap-3">
                     ...
                     <p>We couldn't find any servers for this episode right now.</p>
                   </div>
                 )}
               </div>
             </div>
                     ...
                     <button
                       ...
                       ref={(el) => {
                         if (el && selectedEpisode?.id === ep.id) {
                            ...
                            if (parent) {
                               ...
                               if (top < parent.scrollTop || top > parent.scrollTop + parent.clientHeight - el.clientHeight) {
                                  parent.scrollTo({ top: top - 20, behavior: "smooth" });
                               }
                            }
                         }
                       }}
                       ...
                       className={`border rounded-xl p-3 flex flex-col items-center gap-2 transition-all group outline-none focus-visible:ring-2 focus-visible:ring-[#FF3E3E] ${
                         selectedEpisode?.id === ep.id 
                           ? "bg-[#FF3E3E]/20 border-[#FF3E3E] shadow-[0_0_10px_rgba(255,62,62,0.2)] text-[#FF3E3E]" 
                           : "bg-[#121212] hover:bg-[#FF3E3E]/10 border-white/10 hover:border-[#FF3E3E]/50 text-[#A0A0A0]"
                       }`}
                     >
                       <span className={`text-xs transition-colors ${selectedEpisode?.id === ep.id ? "text-white font-bold" : "group-hover:text-[#FF3E3E]"}`}>
                         Episode {ep.number}
                       </span>
                     </button>
                   ))}
             ...
             {animeInfo && animeInfo.recommended && animeInfo.recommended.length > 0 && (
               <div className="mb-12">
                 <h2 className="text-xl font-bold text-white mb-6 border-l-4 border-[#FF3E3E] pl-3">Recommended For You</h2>
                 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                   {animeInfo.recommended.map((anime) => (
          ...
          >
            ...
            >
              ...
              <p className="text-[#A0A0A0] text-sm mb-6 leading-relaxed">
                We've extracted the raw HLS (M3U8) stream for this episode. You can copy this link and paste it into any web-based downloader or media player (like VLC) to capture it offline.
              </p>
              ...
              <div className="bg-[#050505] border border-white/10 rounded-xl p-3 flex items-center justify-between gap-3 mb-6">
                ...
                <button
                  ...
                  title="Copy link"
                >
                  {downloadLinkCopied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>
